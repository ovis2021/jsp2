...
<body> <!-- 메인 글 : 익명으로 예시(글비밀번호 사용합니다.), 댓글 : 실명으로 예시(로그인해야 글 작성, 삭제할 수 있습니다.) -->
<main>
			...
			<li>
				<ul style="display: flex;">
					...
					<li style="align-self: center;margin-bottom: 20px;">
						<!-- sessionScope.user는 session 에 저장된 애트리뷰트 user 를 가져옵니다.
						sessionScope 을 생략할 수 있지만   request, application 등 다른 영역에 중복된 이름을
						고려하여 명시적으로 지정합니다.-->
	...
	<!-- modal : alert,confirm 함수를 사용하지않고 추가적인 정보를 받을 때 사용자가 만드는 입력 상자...-->
...
<!-- 처음에는 display가 none 이고 안보입니다. -->
	...
	const vclose = document.getElementsById('close');
	vclose.onclick = function(){
		documment.getElementById('myModal').style.display='none';	
	}
	
	//메인글 삭제 -> 모달 입력 표시
	
	function deleteSet() {
		documment.getElementById('myModal').style.display='block';
	}
	...
	if('${sessionScope.user}' != null){	//로그인한 사용자의 이메일을 작성자 입력란에 표시하기(읽기 전용)
	...
	function login(){
		...
		// 다시 페이지로 돌아오기 위해 필요한 값mref,page 요소 값을 전달.(textarea는 불필요.)
